...
angular.module('ion-autocomplete', []).directive('ionAutocomplete', [
    '$ionicBackdrop', '$ionicScrollDelegate', '$document', '$q', '$parse', '$interpolate', '$ionicPlatform', '$compile', '$templateRequest', '$state', '$ionicHistory','$rootScope',
    function ($ionicBackdrop, $ionicScrollDelegate, $document, $q, $parse, $interpolate, $ionicPlatform, $compile, $templateRequest, $state, $ionicHistory,$rootScope) {
        return {
            ...
            controller: ['$attrs', '$timeout', '$scope', function ($attrs, $timeout, $scope) {
                ...
                $timeout(function () {
                    ...
                    controller.selectItemsLabel = valueOrDefault(controller.selectItemsLabel, "Hacer búsqueda...");
            ...
            link: function (scope, element, attrs, controllers) {
                ...
                var template = [
                    ...
                    '<form ng-submit="viewModel.doSearch()"><input type="search" class="ion-autocomplete-search" ng-model="viewModel.searchQuery" ng-model-options="viewModel.ngModelOptions" placeholder="{{viewModel.placeholder}}" auto-focus id="search-autocomplete-input"/><input type="submit" style="position: absolute; left: -9999px; width: 1px; height: 1px;"/></form>',
                    '</label>',
                    ...
                    '<button class="ion-autocomplete-cancel button" ng-click="viewModel.cancelClick()"><i class="icon ion-android-cancel"></i></button>',
                    '</div>',
                ...
                $q.when().then(function () {
                    ...
                    ionAutocompleteController.doSearch = function () {


                        //window.location = '#/app/search/'+ionAutocompleteController.searchQuery;
                        if($state.current.name == 'tabs.nearby'){
                            $rootScope.searchQuery = ionAutocompleteController.searchQuery;
                        }
                        else{
                            $ionicHistory.nextViewOptions({
                                disableBack: true
                            });
                            $state.go('tabs.search_outside', {search_query: ionAutocompleteController.searchQuery});
                        }
